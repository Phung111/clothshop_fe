                      ...
                      <React.Fragment key={item.id}>
                        <Part onClick={() => handleToProductDetail(item.product.id)}>
                          ...
                          <div className="flex h-full w-full items-center justify-center gap-2">
                            {item.product.discountResDTO && <p className="text-sm capitalize text-primary/50 line-through">{window.formatCurrency2(item.product.price)}</p>}
                            <p className="text-sm capitalize">{window.formatCurrency2(item.product.priceTotal)}</p>
                          </div>
                ...
                <div className="flex">
                  ...
                  <div className="flex w-full flex-col items-end gap-3">
                    <div className="flex items-center gap-5">
                      ...
                      <p className="flex w-[200px] items-center justify-end text-sm">{window.formatCurrency2(order.orderItemsTotal)}</p>
                    </div>
                    <div className="flex items-center gap-5">
                      <p className="flex w-[150px] items-center justify-end text-sm text-black/50">Shipping Total:</p>
                      <p className="flex w-[200px] items-center justify-end text-sm">{window.formatCurrency2(order.shipTotal)}</p>
                    </div>
                    <div className="flex items-center gap-5">
                      <p className="flex w-[150px] items-center justify-end text-sm text-black/50">Voucher Total:</p>
                      <p className="flex w-[200px] items-center justify-end text-sm">-{window.formatCurrency2(order.voucherTotal)}</p>
                    </div>
                    <div className="flex items-center gap-5">
                      <p className="flex w-[150px] items-center justify-end text-sm text-black/50">Total Payment:</p>
                      <p className="flex w-[200px] items-center justify-end text-[28px] leading-[28px] text-primary">{window.formatCurrency2(order.total)}</p>
                    </div>
                  </div>
                </div>
              </div>
